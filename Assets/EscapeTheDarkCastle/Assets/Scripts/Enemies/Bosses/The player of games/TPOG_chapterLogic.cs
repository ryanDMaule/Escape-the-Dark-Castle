...
public class TPOG_chapterLogic : ChapterLogicBase
{
    ...
    private List<Card> playerItems = new List<Card>();
    public void RedistributeItems()
    {
        //add all items ot a temp list
        foreach(var player in MainManager.Instance.Players)
        {
            player.AddItemsToList(playerItems);
        }

        //order them on size (larger items first)
        List<Card> playerItemsOrdered = playerItems.OrderBy(o => o.size).ToList();
        playerItemsOrdered.Reverse();

        //assign them randomly to playes that can accept them
        foreach(var card in playerItemsOrdered)
        {
            assignItem(card);
        }
    }

    private bool assignItem(Card card)
    {
        //range of players
        List<PlayerBase> tempPlayersList = new();
        foreach(var player in MainManager.Instance.Players)
        {
            tempPlayersList.Add(player);
        }

        //try and assign the card
        bool cardAssigned = false;

        while (!cardAssigned)
        {
            //random player
            PlayerBase randomPlayer = tempPlayersList[UnityEngine.Random.Range(0, tempPlayersList.Count)];

            if (randomPlayer.addInventoryItem(card))
            {
                return true;
            } else
            {
                tempPlayersList.Remove(randomPlayer);
            }
        }
        return false;
