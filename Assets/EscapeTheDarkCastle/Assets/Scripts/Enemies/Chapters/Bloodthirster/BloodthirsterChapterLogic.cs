...
public class BloodthirsterChapterLogic : ChapterLogicBase
{
    ...
    public void startEnemyTurnPhase()
    {
        if (!enemyBase.enemyDead())
        {
            ...
            //determine who has the lowest health (who isnt resting)
            int lowestHealthValue = 18;
            List<PlayerBase> lowestHealthPlayers = new();
            foreach (var player in MainManager.Instance.Players)
            {
                if (!player.getIsRestingState())
                {
                    if (player.currentHealth == lowestHealthValue)
                    {
                    ...
                    if (player.currentHealth < lowestHealthValue)
                    {
                        ...
                        lowestHealthPlayers.Add(player);
                        lowestHealthValue = player.currentHealth;
                    }
                } else
                {
                    continue;
            ...
            setEnemyTurnHUD();
            foreach (var player in lowestHealthPlayers)
            {
