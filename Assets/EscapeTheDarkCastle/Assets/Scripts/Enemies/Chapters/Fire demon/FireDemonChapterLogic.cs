...
public class FireDemonChapterLogic : ChapterLogicBase
{
    ...
    {
        var enemyDead = enemyBase.enemyDead();
        if (!enemyDead)
        {
    ...
    public override void setWinHUD()
    {
        if(state != BattleState.WON)
        {
            ...
            //reduce the health of the player that kills the enemy
            if (optionSelected == 2)
            {
                MainManager.Instance.playerTurn.RedcuceHealth(1);
            }
        }
